...
- `script-original.md`、`script-revised.md` 和 `script-edit-notes.md` 存在且互相对应。
- 旁白经过人性化处理，适合大声朗读，没有明显套话、宣传腔或空泛结尾。
- 文案改写没有改变用户观点，也没有添加虚构经历、来源、反馈、测试结果或产品能力。
...
- 制作目录中的 `assets/asset-manifest.md` 中的素材均能追溯到来源、授权、采集日期和数据口径。
...
- 旁白、字幕和画面提示是否分工清楚，没有把画面提示读出来。

## 技术检查

两版最终视频都必须满足：

- 画幅与分辨率严格等于 `design-spec.md` 所选格式：横屏 16:9 1920×1080、竖屏 9:16 1080×1920、方形 1:1 1080×1080；统一 30fps，总帧数与时间线一致。
...
- 启用 BGM 时，带 BGM / 无 BGM 两版的画面和 SFX 逐帧一致，视频流 MD5 相同。
- 当前参考工程只直接支持 16:9；9:16 和 1:1 必须提供已验证的独立布局和渲染结果。
